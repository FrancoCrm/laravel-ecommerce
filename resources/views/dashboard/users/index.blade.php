                    ...
                    <table class="max-w-full divide-y divide-gray-200 w-full border text-sm text-left text-gray-700">
                        <thead class="bg-gray-100 text-xs uppercase">
                            <tr>
                                ...
                                <th class="px-4 py-2 border">Diperbarui Pada</th>
                        ...
                        <tbody>
                            @forelse ($users as $user)
                                <tr>
                                    ...
                                    <td class="px-4 py-2 border capitalize">{{ $user->name }}</td>
                                    <td class="px-4 py-2 border">{{ $user->email}}</td>

                                    <td class="px-4 py-2 border">{{ $user->role}}</td>
                                    <td class="px-4 py-2 border">{{ $user->created_at->diffForHumans() }}
                                    </td>
                                    <td class="px-4 py-2 border">{{ $user->updated_at->diffForHumans() }}
                                    </td>
